...
| eManager / MSU Scorecard（TW）| `待確認清單_0803` 剩餘 5 項（④ LA MOPEX(K) 是否含 993024／⑤ total MOPEX-Labor(K) 的 CTOS 是否扣 PG／⑥ MOPEX(K)(Include Outsourcing Cost) 是否應含 IC Burning／⑦⑧ 筆誤與命名統一）| —（待業務回覆）| 待人工（**08-06 新列**）：3 項必問已於 08-06 回覆並實作，**其餘 5 項「對方回覆」欄仍空白**。另 🔴 **08-06 三處 SP 修改的執行與驗證狀態未確認** —— `檢查_必問123修正_20260806.sql` 前提為先 ALTER 再 `EXEC ... 2026,7`，但 SP 是否已於正式 `Global_Fin` ALTER、驗證三段差異是否皆為 0，**本輪無法自檔案確認**（`比對結果\` 最新仍 07-15、`報告\` 最新仍 07-08）；⚠️ 必問3 移除 IC/BCM 自動化後兩指標改吃大檔維護值，**需確認大檔端確有維護，否則直接缺值** |
